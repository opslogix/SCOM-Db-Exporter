...
The Alert module exports SCOM alerts as JSON and **pushes** them directly to Grafana Alloy for ingestion into Loki. It includes:
...
The alert exporter **pushes** alerts directly to Alloy's `loki.source.api` endpoint:
- Each new or modified alert is HTTP POSTed as a JSON object
- Alloy receives the alerts and forwards them to Loki
- No scraping/pulling required - the exporter initiates the connection

### Deduplication

The alert exporter includes built-in deduplication:
- Only **new or modified** alerts are pushed to Alloy
- Alerts are tracked by their `LastModified` timestamp
- Closed alerts are cleaned up after `ClosedAlertRetentionMinutes`
- On service restart, all current alerts are pushed once

The `/alerts` endpoint is still available for debugging purposes, returning the current batch of changed alerts.
...
### Alerts (Pushed from Exporter to Alloy)

The SCOM exporter **pushes** alerts directly to Alloy. Configure Alloy to listen for incoming alerts:
